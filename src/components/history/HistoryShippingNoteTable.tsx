...
export interface HistoryShippingNoteTableI {
  records: any;
  isLoading?: boolean;
}

export function HistoryShippingNoteTable(props: HistoryShippingNoteTableI) {
        ...
        <table className="w-full text-sm text-left">
          <thead className="text-xs text-text-base uppercase bg-bg-subtle/60 border-b-2 border-bg-subtle">
            <tr>
              ...
              <th scope="col" className="px-6 py-3 font-bold tracking-wider border-r border-bg-subtle last:border-r-0">Productos</th>
          ...
          <div className="w-full flex justify-center gap-4 p-4 mx-4 my-4 bg-bg-content rounded-lg shadow-sm border border-bg-subtle text-center">
            <div>
                <p className="text-sm text-text-muted">Cantidad de envios: </p>
                <p className="text-lg font-semibold text-text-base">{ records?.data.length }</p>
            </div>
            <div>
                <p className="text-sm text-text-muted">Total de envios: </p>
