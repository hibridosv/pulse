...
export function ReportPurchasesTable(props: ReportPurchasesTableI) {
  ...
  const listItems = records.map((record: any) => {
      const json = record && JSON.parse(record.documento_json);
      // console.log(json);
    return (
    <tr key={json.id} className={`transition-colors duration-150 odd:bg-bg-subtle/40 hover:bg-bg-subtle divide-x divide-bg-subtle text-text-base ${record?.status == 0 && 'bg-red-200'}`}>
      ...
      <td className="px-3 py-2 text-left whitespace-nowrap clickeable" onClick={()=>{ setElement("purchaseSelected", json); openModal("purchasesDetailsModal") }}>
        ...
        <table className="w-full text-sm text-left">
          <thead className="text-xs text-text-base uppercase bg-bg-subtle/60 border-b-2 border-bg-subtle">
            <tr>
              ...
              <th scope="col" className="px-6 py-3 font-bold tracking-wider border-r border-bg-subtle whitespace-nowrap">CF </th>
              <th scope="col" className="px-6 py-3 font-bold tracking-wider border-r border-bg-subtle whitespace-nowrap">Total</th>
              <th scope="col" className="px-6 py-3 font-bold tracking-wider border-r border-bg-subtle whitespace-nowrap">Retenciones</th>
            </tr>
          </thead>
